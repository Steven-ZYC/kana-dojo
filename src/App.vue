...
const memoryRows = computed(() =>
  chartGroup.value === 'basic' ? basicRows : voicedRows,
);
...
const poolSize = computed(
  () => practicePool.value.length * selectedScripts.value.length,
);
const quizAccuracy = computed(() =>
  quizAnswered.value
    ? Math.round((quizScore.value / quizAnswered.value) * 100)
    : 100,
);
...
const normalizedTarget = computed(() =>
  normalizeJapanese(currentSentence.value.target),
);
const normalizedReading = computed(() =>
  normalizeJapanese(currentSentence.value.reading),
);
const normalizedTyped = computed(() => normalizeJapanese(typingText.value));
const romanLetters = computed(() =>
  currentSentence.value.romaji.toLowerCase().replace(/[^a-z]/g, ''),
);
const expectedKey = computed(
  () => romanLetters.value[romanProgress.value] || '',
);
...
function stringAccuracy(value: string, target: string) {
  ...
  return (
    matchingCharacters(value, target) / Math.max(value.length, target.length)
  );
}
...
function bestMatchCounts(value: string) {
  ...
  if (targetMatches >= readingMatches) {
    return {
      ...
      total: Math.max(value.length, normalizedTarget.value.length),
    };
  }
  return {
    matches: readingMatches,
    total: Math.max(value.length, normalizedReading.value.length),
  };
}
...
function finishPractice() {
  ...
  const elapsed = Math.max(
    0.5,
    (typingNow.value - typingStartTime.value) / 1000,
  );
...
function finishChallenge() {
  ...
  typingResult.value = {
    ...
    accuracy: Math.round(
      (challengeCorrectChars.value / Math.max(1, challengeTotalChars.value)) *
        100,
    ),
    elapsed,
      ...
      modelContext.registerTool(
        {
          ...
          title: '开始假名测验',
          description:
            '设置假名、方向和范围，并在页面中开始一轮 10 题的五十音测验。',
          inputSchema: {
            type: 'object',
            properties: {
          ...
          async execute(input: unknown) {
            ...
            if (
              ...
              options.scripts.some((value) => !['hira', 'kata'].includes(value))
            ) {
            ...
            if (
              ...
              options.categories.some(
                (value) => !['basic', 'voiced'].includes(value),
              )
            ) {
            ...
            const requestedRows = options.rows?.length
              ? options.rows
              : allowedRows;
...
onMounted(() => {
  ...
  document
    ...
    ?.setAttribute('content', darkMode.value ? '#101a20' : '#f6f3ed');
  ...
  <div class="app-shell">
    <header class="topbar">
      ...
      <nav class="primary-nav" aria-label="主要功能">
        ...
        <button
          ...
          @click="openSection('quiz')"
        >
          <Dumbbell :size="18" />练习
        </button>
        <button
          type="button"
          :class="{ active: activeSection === 'typing' }"
          :aria-current="activeSection === 'typing' ? 'page' : undefined"
          @click="openSection('typing')"
        >
          <Keyboard :size="18" />打字
        </button>
      </nav>
    ...
    <main>
      <section
        ...
        class="workspace memory-workspace"
      ...
      >
        ...
        <div class="chart-toolbar" aria-label="五十音图筛选">
          <div class="segmented" role="group" aria-label="五十音类别">
            <button
              type="button"
              :class="{ active: chartGroup === 'basic' }"
              :aria-pressed="chartGroup === 'basic'"
              @click="chartGroup = 'basic'"
            >
              清音
            </button>
            <button
              type="button"
              :class="{ active: chartGroup === 'voiced' }"
              :aria-pressed="chartGroup === 'voiced'"
              @click="chartGroup = 'voiced'"
            >
              浊音・半浊音
            </button>
          </div>
          ...
          >
            ...
            <template v-for="(cell, index) in row.cells" :key="index">
              <div v-if="cell" class="kana-cell" role="cell">
                <span v-if="scriptView !== 'kata'" class="kana hira">{{
                  cell.hira
                }}</span>
                <span v-if="scriptView !== 'hira'" class="kana kata">{{
                  cell.kata
                }}</span>
                <span
                  class="romaji"
                  :class="{ hidden: hideRomaji }"
                  :aria-hidden="hideRomaji"
                  >{{ cell.roma }}</span
                >
              </div>
              <div
                v-else
                class="kana-cell empty"
                role="cell"
                aria-hidden="true"
              >
                ·
        ...
        <p class="chart-footnote">
          现代日语常用 46 个清音；历史假名「ゐ／ヰ」「ゑ／ヱ」未列入常用表。
        </p>
      </section>

      <section
        v-else-if="activeSection === 'quiz'"
        class="workspace"
      ...
      >
        <div class="section-heading compact-heading">
          ...
          <div class="mode-switch" role="group" aria-label="练习方向">
            <button
              type="button"
              :class="{ active: practiceMode === 'kana-romaji' }"
              :aria-pressed="practiceMode === 'kana-romaji'"
              @click="setPracticeMode('kana-romaji')"
            >
              假名 → 罗马字
            </button>
            <button
              type="button"
              :class="{ active: practiceMode === 'romaji-kana' }"
              :aria-pressed="practiceMode === 'romaji-kana'"
              @click="setPracticeMode('romaji-kana')"
            >
              罗马字 → 假名
            </button>
          </div>
        ...
        <div class="practice-layout">
          <aside class="scope-panel" aria-labelledby="scope-title">
            ...
            <fieldset>
              ...
              <div class="row-chip-grid">
                <label
                  ...
                  :key="row.id"
                  :class="{ muted: !selectedCategories.includes(row.category) }"
                >
                  <input
            ...
            <div class="pool-summary">
              <span>当前题库</span><strong>{{ poolSize }}</strong
              ><span>种组合</span>
            </div>
            <button
          ...
          <div class="quiz-stage">
            <div class="quiz-stats" aria-label="本轮成绩">
              <div>
                <Target :size="18" /><span>进度</span
                ><strong>{{ quizAnswered }}/10</strong>
              </div>
              <div>
                <Check :size="18" /><span>正确率</span
                ><strong>{{ quizAccuracy }}%</strong>
              </div>
              <div>
                <Flame :size="18" /><span>连续答对</span
            ...
            <div v-if="quizFinished" class="quiz-card result-card">
              ...
              <p>
                本轮答对 <strong>{{ quizScore }}/10</strong>，正确率
                <strong>{{ quizAccuracy }}%</strong>。
              </p>
              ...
              <button class="button primary" @click="resetQuiz">
                <RotateCcw :size="18" />再练一轮
              </button>
            </div>
            ...
            <div v-else-if="currentQuestion" class="quiz-card">
              <div class="question-meta">
                ...
                <span>{{
                  currentQuestion.script === 'hira' ? '平假名' : '片假名'
                }}</span>
              </div>
              ...
              <div class="answer-area">
                ...
                <div
                  class="answer-row"
                  :class="{
                    correct: quizFeedback?.correct,
                    incorrect: quizFeedback && !quizFeedback.correct,
                  }"
                >
                  <input
                    id="quiz-answer"
                ...
                <p
                  ...
                  :class="{ success: quizFeedback.correct }"
                  role="status"
                  aria-live="polite"
                >
                  {{
              ...
              <button
                class="button primary quiz-submit"
                :disabled="!quizAnswer.trim() && !quizFeedback"
                @click="submitQuiz"
              >
                {{
                  quizFeedback
      ...
      <section v-else class="workspace" aria-labelledby="typing-title">
        <div class="section-heading compact-heading">
          ...
          <div class="mode-switch" role="group" aria-label="打字训练模式">
            <button
              type="button"
              :class="{ active: typingMode === 'practice' }"
              :aria-pressed="typingMode === 'practice'"
              @click="switchTypingMode('practice')"
            >
              引导练习
            </button>
            <button
              type="button"
              :class="{ active: typingMode === 'challenge' }"
              :aria-pressed="typingMode === 'challenge'"
              @click="switchTypingMode('challenge')"
            >
              60 秒挑战
            </button>
          </div>
        ...
        <div class="typing-layout">
          <div class="typing-stage">
            <div class="typing-topline">
              <div class="sentence-count">
                <span>{{ currentSentence.level }}</span> 第
                {{ sentenceIndex + 1 }}/{{ sentences.length }} 句
              </div>
              <div
                v-if="typingMode === 'challenge'"
                class="challenge-clock"
                :class="{ urgent: challengeRemaining <= 10 }"
                role="timer"
                :aria-label="`挑战剩余 ${challengeRemaining} 秒`"
              >
                <Clock3 :size="18" />00:{{
                  String(challengeRemaining).padStart(2, '0')
                }}
              </div>
            ...
            <div class="sentence-card">
              <p class="sentence-jp" lang="ja">
                <template
                  v-for="(segment, index) in currentSentence.segments"
                  :key="index"
                >
                  <ruby v-if="segment.ruby"
                    >{{ segment.text }}<rt>{{ segment.ruby }}</rt></ruby
                  >
            ...
            >
              ...
              <div class="next-key">
                下一键
                <kbd>{{ expectedKey ? expectedKey.toUpperCase() : '✓' }}</kbd>
              </div>
            </div>
            ...
            <div v-if="typingMode === 'practice'" class="typing-actions">
              ...
              <button v-else class="button primary" @click="clearTypingAttempt">
                <RotateCcw :size="17" />再练一次
              </button>
            </div>
            <div v-else class="typing-actions">
              <template v-if="!challengeRunning">
                <button
                  class="button primary challenge-start"
                  @click="startChallenge"
                >
                  <Trophy :size="18" />{{
                    challengeDone ? '再挑战一次' : '开始 60 秒挑战'
                  }}
                </button>
              </template>
              <template v-else>
                <button class="button ghost" @click="skipChallengeSentence">
                  跳过此句
                </button>
                <button class="button secondary" @click="finishChallenge">
                  提前结束
                </button>
              </template>
            </div>
            ...
            >
              ...
              <p>
                键盘只提示罗马字按键；空格选词、Enter 确认交给你的日语输入法。
              </p>
            </div>
          </div>
          ...
          <aside class="metrics-panel" aria-label="打字测算结果">
            ...
            <div class="metric-list">
              <div>
                <span>准确率</span
                ><strong>{{ typingResult?.accuracy ?? liveAccuracy }}%</strong>
              </div>
              <div>
                <span>用时</span
                ><strong
                  >{{
                    (typingResult?.elapsed ?? typingElapsed).toFixed(1)
                  }}s</strong
                >
              </div>
              <div v-if="typingMode === 'challenge'">
                <span>完成句子</span><strong>{{ challengeCompleted }}</strong>
              </div>
              <div v-else>
                <span>罗马字进度</span
                ><strong>{{ romanProgress }}/{{ romanLetters.length }}</strong>
              </div>
            </div>
            ...
            <p v-if="typingResult" class="result-note">
              {{
                typingResult.accuracy >= 95
                  ? '准确又流畅，继续保持。'
                  : typingResult.accuracy >= 80
                    ? '速度不错，再留意错字。'
                    : '先放慢一点，准确会带来速度。'
              }}
            </p>
    ...
    <footer>
      <span>かな道场</span>
      <p>每天十分钟，把假名练成直觉。</p>
      <button type="button" @click="openSection('memory')">
        返回五十音图 ↑
      </button>
    </footer>
  </div>
